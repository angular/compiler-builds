 ...
 * Use of this source code is governed by an MIT-style license that can be
 * found in the LICENSE file at https://angular.io/license
 */var CONTEXT_NAME="ctx",TEMPORARY_NAME="_t",MEANING_SEPARATOR$1="|",ID_SEPARATOR$1="@@";function compileDirective(outputCtx,directive,reflector,bindingParser){var definitionMapValues=[],field=function(key,value){value&&definitionMapValues.push({key:key,value:value,quoted:!1})};field("type",outputCtx.importExpr(directive.type.reference)),field("selectors",createDirectiveSelector(directive.selector)),field("factory",createFactory(directive.type,outputCtx,reflector,directive.queries)),field("hostBindings",createHostBindingsFunction(directive,outputCtx,bindingParser)),field("attributes",function(directiveMetadata,outputCtx){for(var values=[],attributes=directiveMetadata.hostAttributes,_i=0,_a=Object.getOwnPropertyNames(attributes);_i<_a.length;_i++){var key=_a[_i],value=attributes[key];values.push(literal(key),literal(value))}if(values.length>0)return outputCtx.constantPool.getConstLiteral(literalArr(values));return null}(directive,outputCtx)),field("inputs",conditionallyCreateMapObjectLiteral(directive.inputs)),field("outputs",conditionallyCreateMapObjectLiteral(directive.outputs));var className=identifierName(directive.type);className||error("Cannot resolver the name of "+directive.type);var definitionField=outputCtx.constantPool.propertyNameOf(1),definitionFunction=importExpr(Identifiers$1.defineDirective).callFn([literalMap(definitionMapValues)]);outputCtx.statements.push(new ClassStmt(className,null,[new ClassField(definitionField,INFERRED_TYPE,[StmtModifier.Static],definitionFunction)],[],new ClassMethod(null,[],[]),[]))}function unsupported(feature){if(this)throw new Error("Builder "+this.constructor.name+" doesn't support "+feature+" yet");throw new Error("Feature "+feature+" is not supported yet")}var BINDING_INSTRUCTION_MAP=((_a={})[PropertyBindingType$1.Property]=Identifiers$1.elementProperty,_a[PropertyBindingType$1.Attribute]=Identifiers$1.elementAttribute,_a[PropertyBindingType$1.Class]=Identifiers$1.elementClassNamed,_a[PropertyBindingType$1.Style]=Identifiers$1.elementStyleNamed,_a);function interpolate(args){switch((args=args.slice(1)).length){case 3:return importExpr(Identifiers$1.interpolation1).callFn(args);case 5:return importExpr(Identifiers$1.interpolation2).callFn(args);case 7:return importExpr(Identifiers$1.interpolation3).callFn(args);case 9:return importExpr(Identifiers$1.interpolation4).callFn(args);case 11:return importExpr(Identifiers$1.interpolation5).callFn(args);case 13:return importExpr(Identifiers$1.interpolation6).callFn(args);case 15:return importExpr(Identifiers$1.interpolation7).callFn(args);case 17:return importExpr(Identifiers$1.interpolation8).callFn(args)}return args.length>=19&&args.length%2==1||error("Invalid interpolation argument length "+args.length),importExpr(Identifiers$1.interpolationV).callFn([literalArr(args)])}var pipeBindingIdentifiers=[Identifiers$1.pipeBind1,Identifiers$1.pipeBind2,Identifiers$1.pipeBind3,Identifiers$1.pipeBind4];var pureFunctionIdentifiers=[Identifiers$1.pureFunction0,Identifiers$1.pureFunction1,Identifiers$1.pureFunction2,Identifiers$1.pureFunction3,Identifiers$1.pureFunction4,Identifiers$1.pureFunction5,Identifiers$1.pureFunction6,Identifiers$1.pureFunction7,Identifiers$1.pureFunction8];function getLiteralFactory(outputContext,literal$$1){var _a=outputContext.constantPool.getLiteralFactory(literal$$1),literalFactory=_a.literalFactory,literalFactoryArguments=_a.literalFactoryArguments;return literalFactoryArguments.length>0||error("Expected arguments to a literal factory function"),importExpr(pureFunctionIdentifiers[literalFactoryArguments.length]||Identifiers$1.pureFunctionV).callFn([literalFactory].concat(literalFactoryArguments))}function noop(){}var BindingScope=function(){function BindingScope(parent,declareLocalVarCallback){void 0===parent&&(parent=null),void 0===declareLocalVarCallback&&(declareLocalVarCallback=noop),this.parent=parent,this.declareLocalVarCallback=declareLocalVarCallback,this.map=new Map,this.referenceNameIndex=0}return BindingScope.prototype.get=function(name){for(var current=this;current;){var value=current.map.get(name);if(null!=value)return current!==this&&(value={lhs:value.lhs,rhs:value.rhs,declared:!1},this.map.set(name,value)),value.rhs&&!value.declared&&(this.declareLocalVarCallback(value.lhs,value.rhs),value.declared=!0),value.lhs;current=current.parent}return null},BindingScope.prototype.set=function(name,lhs,rhs){return!this.map.has(name)||error("The name "+name+" is already defined in scope to be "+this.map.get(name)),this.map.set(name,{lhs:lhs,rhs:rhs,declared:!1}),this},BindingScope.prototype.getLocal=function(name){return this.get(name)},BindingScope.prototype.nestedScope=function(declareCallback){return new BindingScope(this,declareCallback)},BindingScope.prototype.freshReferenceName=function(){for(var current=this;current.parent;)current=current.parent;return"_r"+current.referenceNameIndex++},BindingScope.ROOT_SCOPE=(new BindingScope).set("$event",variable("$event")),BindingScope}(),TemplateDefinitionBuilder=function(){function TemplateDefinitionBuilder(outputCtx,constantPool,reflector,contextParameter,parentBindingScope,level,contextName,templateName,viewQueries,directiveMatcher,directives,pipeTypeByName,pipes){void 0===level&&(level=0);var _this=this;this.outputCtx=outputCtx,this.constantPool=constantPool,this.reflector=reflector,this.contextParameter=contextParameter,this.level=level,this.contextName=contextName,this.templateName=templateName,this.viewQueries=viewQueries,this.directiveMatcher=directiveMatcher,this.directives=directives,this.pipeTypeByName=pipeTypeByName,this.pipes=pipes,this._dataIndex=0,this._bindingContext=0,this._prefixCode=[],this._creationCode=[],this._variableCode=[],this._bindingCode=[],this._postfixCode=[],this._temporary=temporaryAllocator(this._prefixCode,TEMPORARY_NAME),this._projectionDefinitionIndex=-1,this._unsupported=unsupported,this._inI18nSection=!1,this._i18nSectionIndex=-1,this._phToNodeIdxes=[{}],this.visitReference=invalid$1,this.visitVariable=invalid$1,this.visitAttribute=invalid$1,this.visitBoundAttribute=invalid$1,this.visitBoundEvent=invalid$1,this._bindingScope=parentBindingScope.nestedScope(function(lhsVar,expression){_this._bindingCode.push(lhsVar.set(expression).toDeclStmt(INFERRED_TYPE,[StmtModifier.Final]))}),this._valueConverter=new ValueConverter(outputCtx,function(){return _this.allocateDataSlot()},function(name,localName,slot,value){var pipeType=pipeTypeByName.get(name);pipeType&&_this.pipes.add(pipeType),_this._bindingScope.set(localName,value),_this._creationCode.push(importExpr(Identifiers$1.pipe).callFn([literal(slot),literal(name)]).toStmt())})}return TemplateDefinitionBuilder.prototype.buildTemplateFunction=function(nodes,variables,hasNgContent,ngContentSelectors){void 0===hasNgContent&&(hasNgContent=!1),void 0===ngContentSelectors&&(ngContentSelectors=[]);for(var _i=0,variables_1=variables;_i<variables_1.length;_i++){var variable$$1=variables_1[_i],variableName=variable$$1.name,expression=variable(this.contextParameter).prop(variable$$1.value||"$implicit"),scopedName=this._bindingScope.freshReferenceName();this._bindingScope.set(variableName,variable(variableName+scopedName),expression)}if(hasNgContent){this._projectionDefinitionIndex=this.allocateDataSlot();var parameters=[literal(this._projectionDefinitionIndex)];if(ngContentSelectors.length>1){var r3Selectors=ngContentSelectors.map(function(s){return parseSelectorToR3Selector(s)}),parsed=this.outputCtx.constantPool.getConstLiteral(asLiteral(r3Selectors),!0),unParsed=this.outputCtx.constantPool.getConstLiteral(asLiteral(ngContentSelectors),!0);parameters.push(parsed,unParsed)}this.instruction.apply(this,[this._creationCode,null,Identifiers$1.projectionDef].concat(parameters))}for(var _a=0,_b=this.viewQueries;_a<_b.length;_a++){var query=_b[_a],querySlot=this.allocateDataSlot(),predicate=getQueryPredicate(query,this.outputCtx),args=[literal(querySlot,INFERRED_TYPE),predicate,literal(query.descendants,INFERRED_TYPE)];query.read&&args.push(this.outputCtx.importExpr(query.read.identifier.reference)),this.instruction.apply(this,[this._creationCode,null,Identifiers$1.query].concat(args));var temporary=this._temporary(),getQueryList=importExpr(Identifiers$1.load).callFn([literal(querySlot)]),refresh=importExpr(Identifiers$1.queryRefresh).callFn([temporary.set(getQueryList)]),updateDirective=variable(CONTEXT_NAME).prop(query.propertyName).set(query.first?temporary.prop("first"):temporary);this._bindingCode.push(refresh.and(updateDirective).toStmt())}visitAll$1(this,nodes);for(var creationCode=this._creationCode.length>0?[ifStmt(variable("rf").bitwiseAnd(literal(1),null,!1),this._creationCode)]:[],updateCode=this._bindingCode.length>0?[ifStmt(variable("rf").bitwiseAnd(literal(2),null,!1),this._bindingCode)]:[],_c=0,_d=this._phToNodeIdxes;_c<_d.length;_c++){var phToNodeIdx=_d[_c];if(Object.keys(phToNodeIdx).length>0){var phMap=variable(scopedName=this._bindingScope.freshReferenceName()).set(mapToExpression(phToNodeIdx,!0)).toDeclStmt(INFERRED_TYPE,[StmtModifier.Final]);this._prefixCode.push(phMap)}}return fn([new FnParam("rf",NUMBER_TYPE),new FnParam(this.contextParameter,null)],this._prefixCode.concat(creationCode,this._variableCode,updateCode,this._postfixCode),INFERRED_TYPE,null,this.templateName)},TemplateDefinitionBuilder.prototype.getLocal=function(name){return this._bindingScope.get(name)},TemplateDefinitionBuilder.prototype.visitContent=function(ngContent){var slot=this.allocateDataSlot(),selectorIndex=ngContent.selectorIndex,parameters=[literal(slot),literal(this._projectionDefinitionIndex)],attributeAsList=[];ngContent.attributes.forEach(function(attribute){var name=attribute.name;"select"!==name&&attributeAsList.push(name,attribute.value)}),attributeAsList.length>0?parameters.push(literal(selectorIndex),asLiteral(attributeAsList)):0!==selectorIndex&&parameters.push(literal(selectorIndex)),this.instruction.apply(this,[this._creationCode,ngContent.sourceSpan,Identifiers$1.projection].concat(parameters))},TemplateDefinitionBuilder.prototype.visitElement=function(element){var _this=this,elementIndex=this.allocateDataSlot(),referenceDataSlots=new Map,wasInI18nSection=this._inI18nSection,outputAttrs={},attrI18nMetas={},i18nMeta="";if(this._inI18nSection){var phName=element.name.toLowerCase();this._phToNodeIdxes[this._i18nSectionIndex][phName]||(this._phToNodeIdxes[this._i18nSectionIndex][phName]=[]),this._phToNodeIdxes[this._i18nSectionIndex][phName].push(elementIndex)}for(var _i=0,_a=element.attributes;_i<_a.length;_i++){var attr=_a[_i],name_1=attr.name,value=attr.value;if("i18n"===name_1){if(this._inI18nSection)throw new Error("Could not mark an element as translatable inside of a translatable section");this._inI18nSection=!0,this._i18nSectionIndex++,this._phToNodeIdxes[this._i18nSectionIndex]={},i18nMeta=value}else name_1.startsWith("i18n-")?attrI18nMetas[name_1.slice("i18n-".length)]=value:outputAttrs[name_1]=value}if(this.directiveMatcher){var selector=createCssSelector(element.name,outputAttrs);this.directiveMatcher.match(selector,function(sel,staticType){_this.directives.add(staticType)})}var parameters=[literal(elementIndex),literal(element.name)],i18nMessages=[],attributes=[],hasI18nAttr=!1;Object.getOwnPropertyNames(outputAttrs).forEach(function(name){var value=outputAttrs[name];if(attributes.push(literal(name)),attrI18nMetas.hasOwnProperty(name)){hasI18nAttr=!0;var meta=parseI18nMeta(attrI18nMetas[name]),variable$$1=_this.constantPool.getTranslation(value,meta);attributes.push(variable$$1)}else attributes.push(literal(value))});var attrArg=TYPED_NULL_EXPR;if(attributes.length>0&&(attrArg=hasI18nAttr?getLiteralFactory(this.outputCtx,literalArr(attributes)):this.constantPool.getConstLiteral(literalArr(attributes),!0)),parameters.push(attrArg),element.references&&element.references.length>0){var references=flatten(element.references.map(function(reference){var slot=_this.allocateDataSlot();referenceDataSlots.set(reference.name,slot);var variableName=_this._bindingScope.freshReferenceName();return _this._variableCode.push(variable(variableName,INFERRED_TYPE).set(importExpr(Identifiers$1.load).callFn([literal(slot)])).toDeclStmt(INFERRED_TYPE,[StmtModifier.Final])),_this._bindingScope.set(reference.name,variable(variableName)),[reference.name,reference.value]}));parameters.push(this.constantPool.getConstLiteral(asLiteral(references),!0))}else parameters.push(TYPED_NULL_EXPR);i18nMessages.length>0&&(_b=this._creationCode).push.apply(_b,i18nMessages),this.instruction.apply(this,[this._creationCode,element.sourceSpan,Identifiers$1.createElement].concat(trimTrailingNulls(parameters)));var _b,implicit=variable(CONTEXT_NAME);if(element.outputs.forEach(function(outputAst){var elName=sanitizeIdentifier(element.name),evName=sanitizeIdentifier(outputAst.name),functionName=_this.templateName+"_"+elName+"_"+evName+"_listener",localVars=[],bindingExpr=convertActionBinding(_this._bindingScope.nestedScope(function(lhsVar,rhsExpression){localVars.push(lhsVar.set(rhsExpression).toDeclStmt(INFERRED_TYPE,[StmtModifier.Final]))}),implicit,outputAst.handler,"b",function(){return error("Unexpected interpolation")}),handler=fn([new FnParam("$event",DYNAMIC_TYPE)],localVars.concat(bindingExpr.render3Stmts),INFERRED_TYPE,null,functionName);_this.instruction(_this._creationCode,outputAst.sourceSpan,Identifiers$1.listener,literal(outputAst.name),handler)}),element.inputs.forEach(function(input){input.type===PropertyBindingType$1.Animation&&_this._unsupported("animations");var convertedBinding=_this.convertPropertyBinding(implicit,input.value),instruction=BINDING_INSTRUCTION_MAP[input.type];if(instruction){var value=importExpr(Identifiers$1.bind).callFn([convertedBinding]);_this.instruction(_this._bindingCode,input.sourceSpan,instruction,literal(elementIndex),literal(input.name),value)}else _this._unsupported("binding "+PropertyBindingType$1[input.type])}),this._inI18nSection&&1==element.children.length&&element.children[0]instanceof Text$3){var text=element.children[0];this.visitSingleI18nTextChild(text,i18nMeta)}else visitAll$1(this,element.children);this.instruction(this._creationCode,element.endSourceSpan||element.sourceSpan,Identifiers$1.elementEnd),this._inI18nSection=wasInI18nSection},TemplateDefinitionBuilder.prototype.visitTemplate=function(template){var _this=this,templateIndex=this.allocateDataSlot(),elName="";1===template.children.length&&template.children[0]instanceof Element$1&&(elName=sanitizeIdentifier(template.children[0].name));var contextName=elName?this.contextName+"_"+elName:"",templateName=contextName?contextName+"_Template_"+templateIndex:"Template_"+templateIndex,templateContext="ctx"+this.level,parameters=[literal(templateIndex),variable(templateName),TYPED_NULL_EXPR],attributeNames=[],attributeMap={};if(template.attributes.forEach(function(a){attributeNames.push(asLiteral(a.name),asLiteral("")),attributeMap[a.name]=a.value}),this.directiveMatcher){var selector=createCssSelector("ng-template",attributeMap);this.directiveMatcher.match(selector,function(cssSelector,staticType){_this.directives.add(staticType)})}attributeNames.length&&parameters.push(this.constantPool.getConstLiteral(literalArr(attributeNames),!0)),this.instruction.apply(this,[this._creationCode,template.sourceSpan,Identifiers$1.containerCreate].concat(trimTrailingNulls(parameters)));var context=variable(CONTEXT_NAME);template.inputs.forEach(function(input){var convertedBinding=_this.convertPropertyBinding(context,input.value);_this.instruction(_this._bindingCode,template.sourceSpan,Identifiers$1.elementProperty,literal(templateIndex),literal(input.name),importExpr(Identifiers$1.bind).callFn([convertedBinding]))});var templateFunctionExpr=new TemplateDefinitionBuilder(this.outputCtx,this.constantPool,this.reflector,templateContext,this._bindingScope,this.level+1,contextName,templateName,[],this.directiveMatcher,this.directives,this.pipeTypeByName,this.pipes).buildTemplateFunction(template.children,template.variables);this._postfixCode.push(templateFunctionExpr.toDeclStmt(templateName,null))},TemplateDefinitionBuilder.prototype.visitBoundText=function(text){var nodeIndex=this.allocateDataSlot();this.instruction(this._creationCode,text.sourceSpan,Identifiers$1.text,literal(nodeIndex)),this.instruction(this._bindingCode,text.sourceSpan,Identifiers$1.textCreateBound,literal(nodeIndex),this.convertPropertyBinding(variable(CONTEXT_NAME),text.value))},TemplateDefinitionBuilder.prototype.visitText=function(text){this.instruction(this._creationCode,text.sourceSpan,Identifiers$1.text,literal(this.allocateDataSlot()),literal(text.value))},TemplateDefinitionBuilder.prototype.visitSingleI18nTextChild=function(text,i18nMeta){var meta=parseI18nMeta(i18nMeta),variable$$1=this.constantPool.getTranslation(text.value,meta);this.instruction(this._creationCode,text.sourceSpan,Identifiers$1.text,literal(this.allocateDataSlot()),variable$$1)},TemplateDefinitionBuilder.prototype.allocateDataSlot=function(){return this._dataIndex++},TemplateDefinitionBuilder.prototype.bindingContext=function(){return""+this._bindingContext++},TemplateDefinitionBuilder.prototype.instruction=function(statements,span,reference){for(var params=[],_i=3;_i<arguments.length;_i++)params[_i-3]=arguments[_i];statements.push(importExpr(reference,null,span).callFn(params,span).toStmt())},TemplateDefinitionBuilder.prototype.convertPropertyBinding=function(implicit,value){var _a,convertedPropertyBinding=convertPropertyBinding(this,implicit,value.visit(this._valueConverter),this.bindingContext(),BindingForm.TrySimple,interpolate);return(_a=this._bindingCode).push.apply(_a,convertedPropertyBinding.stmts),convertedPropertyBinding.currValExpr},TemplateDefinitionBuilder}();function getQueryPredicate(query,outputCtx){if(query.selectors.length>1||1==query.selectors.length&&query.selectors[0].value){var selectors=query.selectors.map(function(value){return value.value});return selectors.some(function(value){return!value})&&error("Found a type among the string selectors expected"),outputCtx.constantPool.getConstLiteral(literalArr(selectors.map(function(value){return literal(value)})))}if(1==query.selectors.length){var first=query.selectors[0];if(first.identifier)return outputCtx.importExpr(first.identifier.reference)}return error("Unexpected query form"),NULL_EXPR}function createFactory(type,outputCtx,reflector,queries){for(var args=[],elementRef=reflector.resolveExternalReference(Identifiers.ElementRef),templateRef=reflector.resolveExternalReference(Identifiers.TemplateRef),viewContainerRef=reflector.resolveExternalReference(Identifiers.ViewContainerRef),_i=0,_a=type.diDeps;_i<_a.length;_i++){var dependency=_a[_i],token=dependency.token;if(token){var tokenRef=tokenReference(token);if(tokenRef===elementRef)args.push(importExpr(Identifiers$1.injectElementRef).callFn([]));else if(tokenRef===templateRef)args.push(importExpr(Identifiers$1.injectTemplateRef).callFn([]));else if(tokenRef===viewContainerRef)args.push(importExpr(Identifiers$1.injectViewContainerRef).callFn([]));else if(dependency.isAttribute)args.push(importExpr(Identifiers$1.injectAttribute).callFn([literal(dependency.token.value)]));else{var directiveInjectArgs=[null!=token.identifier?outputCtx.importExpr(tokenRef):literal(tokenRef)],flags=extractFlags(dependency);0!=flags&&directiveInjectArgs.push(literal(flags)),args.push(importExpr(Identifiers$1.directiveInject).callFn(directiveInjectArgs))}}else unsupported("dependency without a token")}for(var queryDefinitions=[],_b=0,queries_1=queries;_b<queries_1.length;_b++){var query=queries_1[_b],predicate=getQueryPredicate(query,outputCtx),parameters=[literal(null,INFERRED_TYPE),predicate,literal(query.descendants)];query.read&&parameters.push(outputCtx.importExpr(query.read.identifier.reference)),queryDefinitions.push(importExpr(Identifiers$1.query).callFn(parameters))}var createInstance=new InstantiateExpr(outputCtx.importExpr(type.reference),args),result=queryDefinitions.length>0?literalArr([createInstance].concat(queryDefinitions)):createInstance;return fn([],[new ReturnStatement(result)],INFERRED_TYPE,null,type.reference.name?type.reference.name+"_Factory":null)}function extractFlags(dependency){var flags=0;return dependency.isHost&&(flags|=1),dependency.isOptional&&(flags|=8),dependency.isSelf&&(flags|=2),dependency.isSkipSelf&&(flags|=4),dependency.isValue&&unsupported("value dependencies"),flags}function trimTrailingNulls(parameters){for(;(exp=parameters[parameters.length-1])instanceof LiteralExpr&&null===exp.value;)parameters.pop();var exp;return parameters}function createDirectiveSelector(selector){return asLiteral(parseSelectorToR3Selector(selector))}function createHostBindingsFunction(directiveMetadata,outputCtx,bindingParser){for(var statements=[],temporary=temporaryAllocator(statements,TEMPORARY_NAME),hostBindingSourceSpan=typeSourceSpan(directiveMetadata.isComponent?"Component":"Directive",directiveMetadata.type),index=0;index<directiveMetadata.queries.length;index++){var query=directiveMetadata.queries[index],getDirectiveMemory=importExpr(Identifiers$1.load).callFn([variable("dirIndex")]),getQueryList=getDirectiveMemory.key(literal(index+1)),assignToTemporary=temporary().set(getQueryList),callQueryRefresh=importExpr(Identifiers$1.queryRefresh).callFn([assignToTemporary]),updateDirective=getDirectiveMemory.key(literal(0,INFERRED_TYPE)).prop(query.propertyName).set(query.first?temporary().prop("first"):temporary()),andExpression=callQueryRefresh.and(updateDirective);statements.push(andExpression.toStmt())}var directiveSummary=directiveMetadata.toSummary(),bindings=bindingParser.createBoundHostProperties(directiveSummary,hostBindingSourceSpan),bindingContext=importExpr(Identifiers$1.load).callFn([variable("dirIndex")]);if(bindings)for(var _i=0,bindings_1=bindings;_i<bindings_1.length;_i++){var bindingExpr=convertPropertyBinding(null,bindingContext,(binding=bindings_1[_i]).expression,"b",BindingForm.TrySimple,function(){return error("Unexpected interpolation")});statements.push.apply(statements,bindingExpr.stmts),statements.push(importExpr(Identifiers$1.elementProperty).callFn([variable("elIndex"),literal(binding.name),importExpr(Identifiers$1.bind).callFn([bindingExpr.currValExpr])]).toStmt())}var eventBindings=bindingParser.createDirectiveHostEventAsts(directiveSummary,hostBindingSourceSpan);if(eventBindings)for(var _a=0,eventBindings_1=eventBindings;_a<eventBindings_1.length;_a++){bindingExpr=convertActionBinding(null,bindingContext,(binding=eventBindings_1[_a]).handler,"b",function(){return error("Unexpected interpolation")});var binding,bindingName=binding.name&&sanitizeIdentifier(binding.name),functionName=(typeName=identifierName(directiveMetadata.type))&&bindingName?typeName+"_"+bindingName+"_HostBindingHandler":null,handler=fn([new FnParam("$event",DYNAMIC_TYPE)],bindingExpr.stmts.concat([new ReturnStatement(bindingExpr.allowDefault)]),INFERRED_TYPE,null,functionName);statements.push(importExpr(Identifiers$1.listener).callFn([literal(binding.name),handler]).toStmt())}if(statements.length>0){var typeName=directiveMetadata.type.reference.name;return fn([new FnParam("dirIndex",NUMBER_TYPE),new FnParam("elIndex",NUMBER_TYPE)],statements,INFERRED_TYPE,null,typeName?typeName+"_HostBindings":null)}return null}var _a,ValueConverter=function(_super){function ValueConverter(outputCtx,allocateSlot,definePipe){var _this=_super.call(this)||this;return _this.outputCtx=outputCtx,_this.allocateSlot=allocateSlot,_this.definePipe=definePipe,_this}return __extends(ValueConverter,_super),ValueConverter.prototype.visitPipe=function(pipe,context){var slot=this.allocateSlot(),slotPseudoLocal="PIPE:"+slot,target=new PropertyRead(pipe.span,new ImplicitReceiver(pipe.span),slotPseudoLocal),bindingId=function(args){return pipeBindingIdentifiers[args.length]||Identifiers$1.pipeBindV}(pipe.args);this.definePipe(pipe.name,slotPseudoLocal,slot,importExpr(bindingId));var value=pipe.exp.visit(this),args=this.visitAll(pipe.args);return new FunctionCall(pipe.span,target,[new LiteralPrimitive(pipe.span,slot),value].concat(args))},ValueConverter.prototype.visitLiteralArray=function(array,context){var _this=this;return new BuiltinFunctionCall(array.span,this.visitAll(array.expressions),function(values){var literal$$1=literalArr(values);return values.every(function(a){return a.isConstant()})?_this.outputCtx.constantPool.getConstLiteral(literal$$1,!0):getLiteralFactory(_this.outputCtx,literal$$1)})},ValueConverter.prototype.visitLiteralMap=function(map,context){var _this=this;return new BuiltinFunctionCall(map.span,this.visitAll(map.values),function(values){var literal$$1=literalMap(values.map(function(value,index){return{key:map.keys[index].key,value:value,quoted:map.keys[index].quoted}}));return values.every(function(a){return a.isConstant()})?_this.outputCtx.constantPool.getConstLiteral(literal$$1,!0):getLiteralFactory(_this.outputCtx,literal$$1)})},ValueConverter}(AstMemoryEfficientTransformer);function invalid$1(arg){throw new Error("Invalid state: Visitor "+this.constructor.name+" doesn't handle "+void 0)}function parserSelectorToR3Selector(selector){var positive=function(selector){var classes=selector.classNames&&selector.classNames.length?[8].concat(selector.classNames):[];return[selector.element&&"*"!==selector.element?selector.element:""].concat(selector.attrs,classes)}(selector),negative=selector.notSelectors&&selector.notSelectors.length?selector.notSelectors.map(function(notSelector){return classes=(selector=notSelector).classNames&&selector.classNames.length?[8].concat(selector.classNames):[],selector.element?[5,selector.element].concat(selector.attrs,classes):selector.attrs.length?[3].concat(selector.attrs,classes):selector.classNames&&selector.classNames.length?[9].concat(selector.classNames):[];var selector,classes}):[];return positive.concat.apply(positive,negative)}function parseSelectorToR3Selector(selector){return CssSelector.parse(selector).map(parserSelectorToR3Selector)}function asLiteral(value){return Array.isArray(value)?literalArr(value.map(asLiteral)):literal(value,INFERRED_TYPE)}function conditionallyCreateMapObjectLiteral(keys){return Object.getOwnPropertyNames(keys).length>0?mapToExpression(keys):null}function mapToExpression(map,quoted){return void 0===quoted&&(quoted=!1),literalMap(Object.getOwnPropertyNames(map).map(function(key){return{key:key,quoted:quoted,value:asLiteral(map[key])}}))}function temporaryAllocator(statements,name){var temp=null;return function(){return temp||(statements.push(new DeclareVarStmt(TEMPORARY_NAME,void 0,DYNAMIC_TYPE)),temp=variable(name)),temp}}function parseI18nMeta(i18n){var meaning,description,id,_a,_b;if(i18n){var idIndex=i18n.indexOf(ID_SEPARATOR$1),descIndex=i18n.indexOf(MEANING_SEPARATOR$1),meaningAndDesc=void 0;meaningAndDesc=(_a=idIndex>-1?[i18n.slice(0,idIndex),i18n.slice(idIndex+2)]:[i18n,""])[0],id=_a[1],meaning=(_b=descIndex>-1?[meaningAndDesc.slice(0,descIndex),meaningAndDesc.slice(descIndex+1)]:["",meaningAndDesc])[0],description=_b[1]}return{description:description,id:id,meaning:meaning}}function createCssSelector(tag,attributes){var cssSelector=new CssSelector;return cssSelector.setElement(tag),Object.getOwnPropertyNames(attributes).forEach(function(name){var value=attributes[name];(cssSelector.addAttribute(name,value),"class"===name.toLowerCase())&&value.trim().split(/\s+/g).forEach(function(className){return cssSelector.addClassName(className)})}),cssSelector}
/**
 * @license
 * Copyright Google Inc. All Rights Reserved.
 *
 ...
 * found in the LICENSE file at https://angular.io/license
 */
var BIND_NAME_REGEXP$1=/^(?:(?:(?:(bind-)|(let-)|(ref-|#)|(on-)|(bindon-)|(@))(.+))|\[\(([^\)]+)\)\]|\[([^\]]+)\]|\(([^\)]+)\))$/,HtmlToTemplateTransform=function(){function HtmlToTemplateTransform(bindingParser){this.bindingParser=bindingParser,this.ngContentSelectors=[],this.hasNgContent=!1}return HtmlToTemplateTransform.prototype.visitElement=function(element){var preparsedElement=preparseElement(element);if(preparsedElement.type===PreparsedElementType.SCRIPT||preparsedElement.type===PreparsedElementType.STYLE)return null;if(preparsedElement.type===PreparsedElementType.STYLESHEET&&isStyleUrlResolvable(preparsedElement.hrefAttr))return null;for(var inlineTemplateSourceSpan,isTemplateElement=isNgTemplate(element.name),matchableAttributes=[],boundProperties=[],boundEvents=[],variables=[],references=[],attributes=[],templateMatchableAttributes=[],templateBoundProperties=[],templateVariables=[],elementHasInlineTemplate=!1,_i=0,_a=element.attrs;_i<_a.length;_i++){var attribute=_a[_i],hasBinding=!1,normalizedName=normalizeAttributeName(attribute.name),isTemplateBinding=!1;if(normalizedName.startsWith("*")){elementHasInlineTemplate&&this.reportError("Can't have multiple template bindings on one element. Use only one attribute prefixed with *",attribute.sourceSpan),isTemplateBinding=!0,elementHasInlineTemplate=!0;var templateBindingsSource=attribute.value,prefixToken=normalizedName.substring("*".length)+":",oldVariables=[];inlineTemplateSourceSpan=attribute.valueSpan||attribute.sourceSpan,this.bindingParser.parseInlineTemplateBinding(prefixToken,templateBindingsSource,attribute.sourceSpan,templateMatchableAttributes,templateBoundProperties,oldVariables),templateVariables.push.apply(templateVariables,oldVariables.map(function(v){return new Variable(v.name,v.value,v.sourceSpan)}))}else hasBinding=this.parseAttribute(isTemplateElement,attribute,matchableAttributes,boundProperties,boundEvents,variables,references);hasBinding||isTemplateBinding||(attributes.push(this.visitAttribute(attribute)),matchableAttributes.push([attribute.name,attribute.value]))}var parsedElement,children=visitAll(preparsedElement.nonBindable?NON_BINDABLE_VISITOR$1:this,element.children);if(preparsedElement.type===PreparsedElementType.NG_CONTENT){this.hasNgContent=!0,element.children&&!element.children.every(isEmptyTextNode)&&this.reportError("<ng-content> element cannot have content.",element.sourceSpan);var selector=preparsedElement.selectAttr,attributes_1=element.attrs.map(function(attribute){return new TextAttribute(attribute.name,attribute.value,attribute.sourceSpan,attribute.valueSpan)}),selectorIndex="*"===selector?0:this.ngContentSelectors.push(selector);parsedElement=new Content(selectorIndex,attributes_1,element.sourceSpan)}else if(isTemplateElement){var boundAttributes=this.createBoundAttributes(element.name,boundProperties);parsedElement=new Template(attributes,boundAttributes,children,references,variables,element.sourceSpan,element.startSourceSpan,element.endSourceSpan)}else{boundAttributes=this.createBoundAttributes(element.name,boundProperties);parsedElement=new Element$1(element.name,attributes,boundAttributes,boundEvents,children,references,element.sourceSpan,element.startSourceSpan,element.endSourceSpan)}if(elementHasInlineTemplate){var attributes_2=[];templateMatchableAttributes.forEach(function(_a){var name=_a[0],value=_a[1];return attributes_2.push(new TextAttribute(name,value,inlineTemplateSourceSpan))});boundAttributes=this.createBoundAttributes("ng-template",templateBoundProperties);parsedElement=new Template(attributes_2,boundAttributes,[parsedElement],[],templateVariables,element.sourceSpan,element.startSourceSpan,element.endSourceSpan)}return parsedElement},HtmlToTemplateTransform.prototype.visitAttribute=function(attribute){return new TextAttribute(attribute.name,attribute.value,attribute.sourceSpan,attribute.valueSpan)},HtmlToTemplateTransform.prototype.visitText=function(text){var valueNoNgsp=replaceNgsp(text.value),expr=this.bindingParser.parseInterpolation(valueNoNgsp,text.sourceSpan);return expr?new BoundText(expr,text.sourceSpan):new Text$3(valueNoNgsp,text.sourceSpan)},HtmlToTemplateTransform.prototype.visitComment=function(comment){return null},HtmlToTemplateTransform.prototype.visitExpansion=function(expansion){return null},HtmlToTemplateTransform.prototype.visitExpansionCase=function(expansionCase){return null},HtmlToTemplateTransform.prototype.createBoundAttributes=function(elementName,boundProperties){var _this=this;return boundProperties.filter(function(prop){return!prop.isLiteral}).map(function(property){var boundProp=_this.bindingParser.createElementPropertyAst(elementName,property);return new BoundAttribute(boundProp.name,boundProp.type,boundProp.securityContext,boundProp.value,boundProp.unit,boundProp.sourceSpan)})},HtmlToTemplateTransform.prototype.parseAttribute=function(isTemplateElement,attribute,matchableAttributes,boundProperties,boundEvents,variables,references){var name=normalizeAttributeName(attribute.name),value=attribute.value,srcSpan=attribute.sourceSpan,bindParts=name.match(BIND_NAME_REGEXP$1),hasBinding=!1;if(bindParts){if(hasBinding=!0,null!=bindParts[1])this.bindingParser.parsePropertyBinding(bindParts[7],value,!1,srcSpan,matchableAttributes,boundProperties);else if(bindParts[2])if(isTemplateElement){var identifier=bindParts[7];this.parseVariable(identifier,value,srcSpan,variables)}else this.reportError('"let-" is only supported on ng-template elements.',srcSpan);else if(bindParts[3]){identifier=bindParts[7];this.parseReference(identifier,value,srcSpan,references)}else if(bindParts[4]){var events=[];this.bindingParser.parseEvent(bindParts[7],value,srcSpan,matchableAttributes,events),addEvents(events,boundEvents)}else if(bindParts[5])this.bindingParser.parsePropertyBinding(bindParts[7],value,!1,srcSpan,matchableAttributes,boundProperties),this.parseAssignmentEvent(bindParts[7],value,srcSpan,matchableAttributes,boundEvents);else if(bindParts[6])this.bindingParser.parseLiteralAttr(name,value,srcSpan,matchableAttributes,boundProperties);else if(bindParts[8])this.bindingParser.parsePropertyBinding(bindParts[8],value,!1,srcSpan,matchableAttributes,boundProperties),this.parseAssignmentEvent(bindParts[8],value,srcSpan,matchableAttributes,boundEvents);else if(bindParts[9])this.bindingParser.parsePropertyBinding(bindParts[9],value,!1,srcSpan,matchableAttributes,boundProperties);else if(bindParts[10]){events=[];this.bindingParser.parseEvent(bindParts[10],value,srcSpan,matchableAttributes,events),addEvents(events,boundEvents)}}else hasBinding=this.bindingParser.parsePropertyInterpolation(name,value,srcSpan,matchableAttributes,boundProperties);return hasBinding},HtmlToTemplateTransform.prototype.parseVariable=function(identifier,value,sourceSpan,variables){identifier.indexOf("-")>-1&&this.reportError('"-" is not allowed in variable names',sourceSpan),variables.push(new Variable(identifier,value,sourceSpan))},HtmlToTemplateTransform.prototype.parseReference=function(identifier,value,sourceSpan,references){identifier.indexOf("-")>-1&&this.reportError('"-" is not allowed in reference names',sourceSpan),references.push(new Reference(identifier,value,sourceSpan))},HtmlToTemplateTransform.prototype.parseAssignmentEvent=function(name,expression,sourceSpan,targetMatchableAttrs,boundEvents){var events=[];this.bindingParser.parseEvent(name+"Change",expression+"=$event",sourceSpan,targetMatchableAttrs,events),addEvents(events,boundEvents)},HtmlToTemplateTransform.prototype.reportError=function(message,sourceSpan,level){void 0===level&&(level=ParseErrorLevel.ERROR),this.errors.push(new ParseError(sourceSpan,message,level))},HtmlToTemplateTransform}(),NON_BINDABLE_VISITOR$1=new(function(){function NonBindableVisitor(){}return NonBindableVisitor.prototype.visitElement=function(ast){var preparsedElement=preparseElement(ast);if(preparsedElement.type===PreparsedElementType.SCRIPT||preparsedElement.type===PreparsedElementType.STYLE||preparsedElement.type===PreparsedElementType.STYLESHEET)return null;var children=visitAll(this,ast.children,null);return new Element$1(ast.name,visitAll(this,ast.attrs),[],[],children,[],ast.sourceSpan,ast.startSourceSpan,ast.endSourceSpan)},NonBindableVisitor.prototype.visitComment=function(comment){return null},NonBindableVisitor.prototype.visitAttribute=function(attribute){return new TextAttribute(attribute.name,attribute.value,attribute.sourceSpan)},NonBindableVisitor.prototype.visitText=function(text){return new Text$3(text.value,text.sourceSpan)},NonBindableVisitor.prototype.visitExpansion=function(expansion){return null},NonBindableVisitor.prototype.visitExpansionCase=function(expansionCase){return null},NonBindableVisitor}());function normalizeAttributeName(attrName){return/^data-/i.test(attrName)?attrName.substring(5):attrName}function addEvents(events,boundEvents){boundEvents.push.apply(boundEvents,events.map(function(e){return new BoundEvent(e.name,e.handler,e.target,e.phase,e.sourceSpan)}))}function isEmptyTextNode(node){return node instanceof Text&&0==node.value.trim().length}
/**
 * @license
 * Copyright Google Inc. All Rights Reserved.
 *
