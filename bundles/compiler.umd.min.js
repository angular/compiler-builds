 ...
 * found in the LICENSE file at https://angular.io/license
 */
function compilePipe(outputCtx,pipe,reflector){var definitionMapValues=[];definitionMapValues.push({key:"type",value:outputCtx.importExpr(pipe.type.reference),quoted:!1});var templateFactory=createFactory(pipe.type,outputCtx,reflector,[]);definitionMapValues.push({key:"factory",value:templateFactory,quoted:!1}),pipe.pure&&definitionMapValues.push({key:"pure",value:literal(!0),quoted:!1});var className=identifierName(pipe.type);className||error("Cannot resolve the name of "+pipe.type),outputCtx.statements.push(new ClassStmt(className,null,[new ClassField(outputCtx.constantPool.propertyNameOf(3),INFERRED_TYPE,[StmtModifier.Static],importExpr(Identifiers$1.definePipe).callFn([literalMap(definitionMapValues)]))],[],new ClassMethod(null,[],[]),[]))}function toTypeScript(file,preamble){if(void 0===preamble&&(preamble=""),!file.stmts)throw new Error("Illegal state: No stmts present on GeneratedFile "+file.genFileUrl);return(new TypeScriptEmitter).emitStatements(file.genFileUrl,file.stmts,preamble)}/**
 * @license
 * Copyright Google Inc. All Rights Reserved.
 *
 * Use of this source code is governed by an MIT-style license that can be
